...
/*namespace DataTypes
{
    public class OrientedBoundingBox
    {
        ...
        {
            return (new BoundingSphere(Vector3.Transform(other.Center, Transforms), other.Radius).Intersects(new BoundingBox(Min,Max)));
        }
        private Matrix toMe;
            
         private Vector3 centerOther;
         private Vector3 extentsOther;
         private Vector3 separation;

         private Matrix3 rotations;
         private Matrix3 absRotations;
        public bool Intersects(OrientedBoundingBox other)
        {
            ...
            toMe = other.Transforms * Matrix.Invert(_transforms);
            
            centerOther = Utility.Multiply(other.Center, toMe);
            extentsOther = other.Extents;
            separation = centerOther - _center;

            rotations = new Matrix3(toMe);
            absRotations = Utility.Abs(rotations);
        ...
        public Vector3[] GetCorners()
        {
            Vector3[] Corners = 
            {
                Vector3.Transform(new Vector3(Max.X,Max.Y,Max.Z)-Center,Transforms* Matrix.CreateTranslation(Center)),
                Vector3.Transform(new Vector3(Min.X,Max.Y,Max.Z)-Center,Transforms* Matrix.CreateTranslation(Center)),
                Vector3.Transform(new Vector3(Min.X,Min.Y,Max.Z)-Center,Transforms* Matrix.CreateTranslation(Center)),
                Vector3.Transform(new Vector3(Max.X,Min.Y,Max.Z)-Center,Transforms* Matrix.CreateTranslation(Center)),
                Vector3.Transform(new Vector3(Max.X,Max.Y,Min.Z)-Center,Transforms* Matrix.CreateTranslation(Center)),
                Vector3.Transform(new Vector3(Min.X,Max.Y,Min.Z)-Center,Transforms* Matrix.CreateTranslation(Center)),
                Vector3.Transform(new Vector3(Min.X,Min.Y,Min.Z)-Center,Transforms* Matrix.CreateTranslation(Center)),
                Vector3.Transform(new Vector3(Max.X,Min.Y,Min.Z)-Center,Transforms* Matrix.CreateTranslation(Center)),
            };
            return (Corners);
...
namespace DataTypes
{
    ...
    /// </summary>
    public class OrientedBoundingBox : IEquatable<OrientedBoundingBox>
    {
        #region Constants
        ...
        /// </summary>
        public static ContainmentType Contains(BoundingBox boxA, OrientedBoundingBox oboxB)
        {
            ...
            return OrientedBoundingBox.ContainsRelativeBox(boxA_halfExtent, oboxB.HalfExtent, mb);
        }
        #endregion
        ...
        /// the given other box.
        /// </summary>
        public ContainmentType Contains(OrientedBoundingBox other)
        {
            // Build the 3x3 rotation matrix that defines the orientation of 'other' relative to this box
        ...
        public Vector3 ContainsNormal(OrientedBoundingBox other)
        {
            // Build the 3x3 rotation matrix that defines the orientation of 'other' relative to this box
            Quaternion invOrient;
            Quaternion.Conjugate(ref Orientation, out invOrient);
            ...
            double AngleMoving = (float)Math.Asin(2 * (other.Orientation.W * other.Orientation.Y - other.Orientation.Z * other.Orientation.X));
            double AngleStable = (float)(Math.Asin(2 * (Orientation.W * Orientation.Y - Orientation.Z * Orientation.X)));
            /*
            Vector3 Rotation = new Vector3(
                (float)Math.Atan2(2 * (O.W * O.X + O.Y * O.Z), 1 - 2 * (O.X * O.X + O.Y * O.Y)),
                (float)Math.Asin(2 * (O.W * O.Y - O.Z * O.X)),
                (float)Math.Atan2(2 * (O.W * O.Z + O.X * O.Y), 1 - 2 * (O.Z * O.Z + O.Y * O.Y)));
             */
            Vector3 Returner = ContainsNormal(HalfExtent, other.HalfExtent, relTransform);
            //USE THIS LINK!!!!!! http://en.wikipedia.org/wiki/Conversion_between_quaternions_and_Euler_angles
            if (Returner == Vector3.One)
                Returner = new Vector3(
                    (float)(Math.Cos(AngleStable) * Math.Cos(AngleMoving - AngleStable)),
                    0f,
                    (float)(Math.Sin(AngleStable) * Math.Cos(AngleMoving - AngleStable)));
            else if (Returner == Vector3.One * 3)
                Returner = new Vector3(
                    (float)(Math.Cos(AngleStable + Math.PI / 2) * Math.Cos(AngleMoving - Math.PI / 2 - AngleStable)),
                    0f,
                    (float)(Math.Sin(AngleStable + Math.PI / 2) * Math.Cos(AngleMoving - Math.PI / 2 - AngleStable)));
            else if (Returner == Vector3.One * 2)
                Returner = new Vector3(
                    (float)(Math.Cos(AngleStable) * Math.Cos(AngleMoving - AngleStable)),
                    0f,
                    (float)(Math.Sin(AngleStable) * Math.Cos(AngleMoving - AngleStable)));
            return new Vector3(Math.Abs(Returner.X), Math.Abs(Returner.Y), Math.Abs(Returner.Z));
        }
        #endregion
        ...
        /// </summary>
        public static ContainmentType Contains(BoundingFrustum frustum, OrientedBoundingBox obox)
        {
        ...
        /// </summary>
        public static ContainmentType Contains(BoundingSphere sphere, OrientedBoundingBox box)
        {
            // Transform the sphere into local box space
        ...
        /// </summary>
        public Vector3[] GetRotatedCorners()
        {
            Vector3[] corners = new Vector3[CornerCount];
            GetRotatedCorners(corners, 0);
            return corners;
        }
        public Vector3[] GetCorners()
        {
        ...
        public void GetRotatedCorners(Vector3[] corners, int startIndex)
        {
        ...
        {
            Vector3 hX = Vector3.UnitX * HalfExtent.X;
            Vector3 hY = Vector3.UnitY * HalfExtent.Y;
            Vector3 hZ = Vector3.UnitZ * HalfExtent.Z;
            int i = startIndex;
            corners[i++] = Center - hX + hY + hZ;
            corners[i++] = Center + hX + hY + hZ;
            corners[i++] = Center + hX - hY + hZ;
        ...
        public static ContainmentType ContainsRelativeBox(Vector3 hA, Vector3 hB, Matrix mB)
        {
        ...
        private static Vector3 ContainsNormal(Vector3 hA, Vector3 hB, Matrix mB)
        {
            Vector3 mB_T = mB.Translation;
            Vector3 mB_TA = new Vector3(Math.Abs(mB_T.X), Math.Abs(mB_T.Y), Math.Abs(mB_T.Z));
            // Transform the extents of B
            ...
            Vector3 B2Center = mB_T;
            if (B2Center.X >= (Math.Abs(hA.X) + Math.Abs(hB.X)))
                return (Vector3.One * 2);
            else if (B2Center.X <= -(Math.Abs(hA.X) + Math.Abs(hB.X)))
                return (Vector3.One * 2);
            else if (B2Center.Y >= (Math.Abs(hA.Y) + Math.Abs(hB.Y)))
                return (Vector3.UnitY);
            else if (B2Center.Y <= -(Math.Abs(hA.Y) + Math.Abs(hB.Y)))
                return (-Vector3.UnitY);
            else if (B2Center.Z >= (Math.Abs(hA.Z) + Math.Abs(hB.Z)))
